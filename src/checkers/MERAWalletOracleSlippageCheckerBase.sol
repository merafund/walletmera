...
/// @notice Shared balance-delta and Chainlink oracle slippage logic for swap checkers.
abstract contract MERAWalletOracleSlippageCheckerBase is
    Ownable,
    Pausable,
    ...
    IMERAWalletUniswapV2SlippageErrors
{
    ...
    function checkBefore(MERAWalletTypes.Call calldata call, bytes32 operationId, uint256 callId)
        ...
        whenNotPaused
    {
        _requireAllowedRouter(call.target, callId);

        MERAWalletUniswapV2SlippageTypes.CheckerDataSlippageCheckData memory decoded = _decodeSwapCheckData(call);
        require(decoded.tokenIn != address(0) && decoded.tokenOut != address(0), SlippageInvalidAddress());
        require(!(decoded.ethIn && decoded.ethOut), InvalidCheckerData());

        address wallet = msg.sender;
        address assetWhitelist = _effectiveAssetWhitelist(wallet);
        _requireAssetsAllowed(assetWhitelist, decoded.tokenIn, decoded.tokenOut, callId);
        _storeSwapSnapshot(
            wallet,
            operationId,
            callId,
    ...
    function checkAfter(MERAWalletTypes.Call calldata call, bytes32 operationId, uint256 callId)
        ...
        whenNotPaused
    {
        _checkSwapAfter(call, operationId, callId);
    }
    ...
    /// Checks only swap endpoint tokens, not intermediate hop tokens.
    function _requireAssetsAllowed(address assetWhitelist, address tokenIn, address tokenOut, uint256 callId)
        internal
        view
    {
