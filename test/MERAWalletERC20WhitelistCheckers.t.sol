...
contract MERAWalletERC20WhitelistCheckersTest is Test {
    ...
    function test_Transfer_HappyPath() public {
        ...
        vm.prank(emergency);
        wallet.setWhitelistedChecker(address(transferChecker), true, abi.encode(_cfg()));
    ...
    function test_Transfer_RevertsWhenTokenNotAllowed() public {
        ...
        vm.prank(emergency);
        wallet.setWhitelistedChecker(address(transferChecker), true, abi.encode(cfg));
    ...
    function test_Transfer_UsesDefaultWhitelistsWhenWalletConfigZero() public {
        ...
        wallet.setWhitelistedChecker(address(transferChecker), true, abi.encode(emptyCfg));
        vm.stopPrank();
    ...
    function test_Approve_HappyPath() public {
        _allowTokenAndCounterparty();
        vm.prank(emergency);
        wallet.setWhitelistedChecker(address(approveChecker), true, abi.encode(_cfg()));
